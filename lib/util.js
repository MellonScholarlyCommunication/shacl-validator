...
function loadCache(cache) {
    ...
    try {
        ...
        logger.error(`failed to parse ${cache}: ${e.message}`);
        return {};
    }
}

// Cache lookup that ignores a trailing-slash difference, so an agent that
// asks for `https://coar-notify.net/` still hits the `https://coar-notify.net`
// entry (and vice versa).
function cacheLookup(cache, url) {
    const alt = url.endsWith('/') ? url.slice(0, -1) : `${url}/`;
    return cache[url] ?? cache[alt];
}

// Always return a loader so rdf-parse never falls back to its own
// network-fetching loader: a missing/broken cache must not bypass safe mode.
function customDocumentLoader(options) {
    const CACHE = loadCache(options?.cache);

    return {
        load: async (url) => {
            const cached = cacheLookup(CACHE, url);
            if (cached) {
