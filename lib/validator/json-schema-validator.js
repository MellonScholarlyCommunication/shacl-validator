...
export class JSONSchemaValidator {
    ...
    async reportAsMarkdown(report) {
        ...
        if (report.errors.length == 0) {
            markdown += `OK - no obvious interoperability issues ${CHECK_CHAR}.\n`;
            return markdown;
        }

        markdown += `WARNING - your input data has some interoperability issues.\n\n`;

        for (let i = 0 ; i < report.errors.length ; i++ ) {
            const error = report.errors[i];
            console.log(JSON.stringify(error,null,2));
            const message = this.#sanitizeError(error.message);
            markdown += `  - ${WARNING_CHAR} **[doc]${error.instancePath.replaceAll("/",ARROW_CHAR)}** : ${message}.\n`;
        }
